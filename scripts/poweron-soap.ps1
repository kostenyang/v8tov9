...
public class TApos:ICertificatePolicy{public bool CheckValidationResult(ServicePoint s,X509Certificate c,WebRequest r,int p){return true;}}
"@
[System.Net.ServicePointManager]::CertificatePolicy=New-Object TApos
...
$po=@"
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="urn:vim25"><soapenv:Body>
<urn:PowerOnVM_Task><urn:_this type="VirtualMachine">vm-29028</urn:_this></urn:PowerOnVM_Task>
</soapenv:Body></soapenv:Envelope>
"@
try{ $r=Invoke-WebRequest -Uri "https://$vc/sdk" -Method Post -Body $po -Headers $hdr -WebSession $sess -UseBasicParsing; Write-Host "task: $(([xml]$r.Content).SelectSingleNode('//*[local-name()=\"returnval\"]').InnerText)" }
