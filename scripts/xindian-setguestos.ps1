...
public class TAsg:ICertificatePolicy{public bool CheckValidationResult(ServicePoint s,X509Certificate c,WebRequest r,int p){return true;}}
"@
[System.Net.ServicePointManager]::CertificatePolicy=New-Object TAsg
...
<urn:spec><urn:guestId>vmkernel8Guest</urn:guestId></urn:spec></urn:ReconfigVM_Task>
</soapenv:Body></soapenv:Envelope>
"@
  try{ $r=Invoke-WebRequest -Uri "https://$vc/sdk" -Method Post -Body $rc -Headers $hdr -WebSession $sess -UseBasicParsing
    ...
    Write-Host "$vm : guestId->vmkernel8Guest task=$tid" }
